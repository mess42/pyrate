...
initial wavevector  & $\Wavevector_{pilot}$ & ${\Wavevector} = \Wavevector_{pilot} + \Delta\Wavevector$ \\
initial ray direction  & $\Vector{d}_{pilot}$ & ${\Vector{d}} = \Vector{d}_{pilot} + \Delta\Vector{d}$ \\
surface normal      & $\Vector{n}_{pilot} = \Vector{n}(\Vector{r}_{pilot})$ & ${\Vector{n}} = \Vector{n}({\Vector{r}}) = \Vector{n}_{pilot} + \Delta\Vector{n}$ \\
& & \\ \hline & & \\
out-of-plane wavevector component &  $\Wavevector_{pilot, \perp} = (\Wavevector_{pilot} \Vector{n})\Vector{n}$ & ${\Wavevector}_{\perp} = ({\Wavevector} {\Vector{n}}){\Vector{n}}$ \\
in-plane wavevector component     &  $\Wavevector_{pilot, \parallel} = \Wavevector_{pilot} - \Wavevector_{pilot, \perp}$ & ${\Wavevector}_{\parallel} = {\Wavevector} - {\Wavevector}_{\perp}$ \\
\end{tabular}
\\[2ex]
...
In this representation, the normalization requirement $|{\Vector{d}}| = 1$ imposes no nonlinear constraint on a third component depending on the choice of $U$ and $V$.
...
\\[2ex]
 \Delta \wavenumber_{2,i} \approx&
   ...
   +& n_{i, pilot} &\Delta\xi 
\end{eqnarray}
Next we try to express $\Delta\Vector{n}$ and $\Delta\xi$ as function of $\Delta\Vector{r}$ and $\Delta\Vector{k}$.
...
\end{eqnarray}
We introduce a scalar function $F$. The function shall represent the surface shape implicitly as $F(\Vector{r}_{pilot}) = 0$. 
One possibility to define $F$ would be $F = z - \textrm{sag}(x,y)$.
\begin{eqnarray}
...
We assume an explicit $\Vector{r}_{pilot}$, an explicit $\Wavevector$, and an implicit $\Vector{r}_{pilot}$ dependence in terms
