...
\\[2ex]
 \Delta \wavenumber_{2,i} \approx&
   ...
   +& n_{i, pilot} &\Delta\xi 
\end{eqnarray}
Next we try to express $\Delta\Vector{n}$ and $\Delta\xi$ as functions of $\Delta\Vector{r}$ and $\Delta\Vector{k}$.
...
One possibile definition is $F = z - \textrm{sag}(x,y)$. We express the surface normal in terms of $F$
\begin{eqnarray}
 n_i &=& \frac{\partial_i F}{\sqrt{(\partial_k F)(\partial_k F)}}
 \\
 \partial_j n_i &=& \frac{\partial_j \partial_\ell F}{\sqrt{(\partial_k F)(\partial_k F)}} \left(\delta_{\ell i} - n_\ell n_i\right)
