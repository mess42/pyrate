...
they become the direction $\vec{k}$ vectors of the plane wave and therefore are responsible for the transverse
...
\begin{align}
 \omega^2 \mu_0 \permittivity(\vec{x}) - (\psi_{,k})^2&= 0\,,
\end{align}
...
\begin{align}
 \left(-\vec{k}^2 \delta_{ij} + k_i k_j + \omega^2 \mu_0 \permittivity_{ij} \right) E_{j\,0} &= 0 \,.
\end{align}
or more explicit
\begin{eqnarray}
\begin{pmatrix}
...
\end{eqnarray}
and holds for any planar boundary between two homogeneous media. $\vec{k}_2$ consists of the
...
we find 
\begin{align}
 \xi(\vec{k}_{1\,\parallel},\permittivity_2) &= \sqrt{\omega^2 \mu_0 \epsilon_2 - \vec{k}_{1\,\parallel}^2}\,.
\end{align}
...
\end{align}
Here $g^{ij}$ is the inverse of $g_{ij}$ such that $g^{ik} g_{kj} = \delta_{ij}$.
...
In a perfect lens, according to the Fermat law, all rays from one object point to an image point have the same optical path length. 
...
For systems with image-sided anisotropic immersion, a perfect lens with zero optical path differences from the object to the image plane would accumulate different path lengths when propagating back to the reference sphere.
Therefore, we perform the back-propagation in vacuum.
...
One can interpret the path differences in the reference sphere as follows:
...
As a shortcut, the electric field map of the emitters in the curved exit pupil is often assumed planar and equidistant and Fourier transformed instead of Huygens propagated.
...
\begin{align}
 \vec{p}_\text{i} &= \begin{pmatrix} x + \sigma d_x \\ y + \sigma d_y \\ -d_1 + \sigma \sqrt{1 - d_x^2 - d_y^2} \end{pmatrix}\,.
\end{align}
...
\newcommand{\dqv}{\Delta \vec{q}}
\newcommand{\dpv}{\Delta \vec{p}}

\newcommand{\dqi}[1]{\Delta q_{#1}}
\newcommand{\dpi}[1]{\Delta p_{#1}}


\newcommand{\qnv}{\vec{q}_0}
\newcommand{\pnv}{\vec{p}_0}

\newcommand{\qni}[1]{q_{0\,#1}}
\newcommand{\pni}[1]{p_{0\,#1}}
Let the optical axis at a certain surface start at $\vec{q}_0$ and $\vec{p}_0$ then (in 3D formalism)
