...
two materials with constant optical indices. Notice that $\sigma$ depends on the starting variables two, e.g., 
for a conic section it is given by (see \eqref{eq:intersectionconicsection})
\begin{align}
...
\end{eqnarray}
In a strictly linear approximation of the new coordinates where we neglect all dependencies of $x$, $y$, $d_x$, $d_y$ in $\sigma$ this gives
\begin{align}
...
\end{align}
If we further assume that we may neglect the normal vector terms in the new direction equation \eqref{eq:newdirection}
which means
\begin{subequations}
 \begin{align}
  {d'}_x &= \frac{n_1}{n_2} d_x\,,\\
  {d'}_y &= \frac{n_1}{n_2} d_y\,,
 \end{align}
\end{subequations}
...
\begin{align}
 x' &\approx x + \left(d_1 + (d_2 + d^2_1 \rho(1+c)) \frac{n_1}{n_2}\right) d_x\,,\\
 y' &\approx y + \left(d_1 + (d_2 + d^2_1 \rho(1+c)) \frac{n_1}{n_2}\right) d_y\,.
\end{align}
\end{subequations}
Therefore the matrix formulation is given by
\begin{align}
 \begin{pmatrix} x' \\ y' \\ {d'}_x \\ {d'}_y \end{pmatrix} &= 
 \begin{pmatrix} 1 & 0 & d_1 \left(1 + d_1 \rho(1+c)\frac{n_1}{n_2}\right) + d_2 \frac{n_1}{n_2} & 0 \\ 0 & 1 & 0 & d_1 \left(1 + d_1 \rho(1+c)\frac{n_1}{n_2}\right) + d_2 \frac{n_1}{n_2} \\ 0 & 0 & \frac{n_1}{n_2} & 0 \\ 0 & 0 & 0 & \frac{n_1}{n_2} \end{pmatrix} 
 \begin{pmatrix} x \\ y \\ d_x \\ d_y \end{pmatrix}\,,
\end{align}
which is a very rough approximation which is only valid directly on the optical axis.
...
\begin{align}
 ...
 \begin{pmatrix} 1 &  d_1 \left(1 + d_1 \rho(1+c)\frac{n_1}{n_2}\right) + d_2 \\ 0 & \frac{n_1}{n_2} \end{pmatrix}
 \begin{pmatrix} r  \\ d \end{pmatrix}\,.
\end{align}
\remark{Perhaps the approximation in the angular parts is too rough!}
