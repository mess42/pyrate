...
it is not known whether there exists such a user-friendly numerically stable form of $t$ like in the former cases.

Let at first $a(x,y) \ge 1$ then \eqref{eq:implicitbiconic} can only be zero if $z = b(x,y)$. This leads to a quadratic equation for $t$ which
has nearly the same structure as shown in the former sections.\footnote{
