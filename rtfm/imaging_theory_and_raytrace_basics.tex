...
\begin{align}
 (1 + c) z^2 - \frac{2}{\rho} z + x^2 + y^2 &=0\,.
\end{align}
...
Unfortunately finding the invariant or implicit form of \eqref{eq:biconic} is not so easy. The simplest implicit form is given by
\begin{align}
 (a(x,y) - 1) z^2 + (z - b(x,y))^2 &= 0\,,
\end{align}
where $a(x,y) = (1 + c_x) \rho_x^2 x^2 + (1 + c_y) \rho_y^2 y^2$ and $b(x,y) = \rho_x x^2 + \rho_y y^2$. This means inserting \eqref{eq:ray}
leads to a fourth order equation, which is in principle analytically solvable. But one the one hand it is very complicated and it is not known
whether there exists such a user-friendly numerically stable form of $t$ like in the former cases.
