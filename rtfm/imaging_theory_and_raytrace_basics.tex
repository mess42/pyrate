...
\begin{eqnarray}
 \Vector{S} &=& \Re \Efield \times \Re \Hfield \\
            ...
            &=& \frac{1}{2} \Re ( \Efield_0 \times \Hfield_0 \exp(-2i\omega t) ) + \frac{1}{2} \Re ( \overline{\Efield}_0 \times \Hfield_0 )
\end{eqnarray}
The fast oscillation of the Poynting vector at $2 \omega$ is not directly measurable at optical frequencies, only its time average $<\Vector{S}>$.

\begin{eqnarray}
 < \Vector{S} >   &=& \frac{\omega}{2\pi} \int_{0}^{2\pi / \omega} dt \, \Vector{S} \\
 < \Vector{S} >   &=& \frac{1}{2} \Re ( \overline{\Efield} \times \Hfield ) \\
 < \Vector{S} >   &=& \frac{1}{2} \Re \left( \overline{\Efield} \times \frac{\Nabla \times \Efield}{i \omega \vacuumpermeability} \right) \\
 < \Vector{S} >_j &=&  \Re \frac{ \overline{\scalarEfield}_i \partial_j \scalarEfield_i - \overline{\scalarEfield}_j \partial_i \scalarEfield_i }{2 i \omega \vacuumpermeability}
\end{eqnarray}
...
\begin{eqnarray}
 \permittivity &=&
 \begin{pmatrix}
  \scalarpermittivity_{ord}  & 0 & 0 \\
  0 & \scalarpermittivity_{ord}  & 0 \\
...
\begin{eqnarray}
\begin{pmatrix}
 \wavenumber_z^2 - \omega^2 \vacuumpermeability \scalarpermittivity_{ord} 
 &
 0
 &
 ...
 0
 &
 \wavenumber_x^2 + \wavenumber_z^2 - \omega^2 \vacuumpermeability \scalarpermittivity_{ord} 
 &
 0 
 \\
 - \wavenumber_x \wavenumber_z 
...
The equation system splits up into a coupled system of $\Efield_x$ and $\Efield_z$ , and an independent equation for $\Efield_y$.
...
&=& 0
\\
&\begin{pmatrix} \wavenumber_x^2 + \wavenumber_z^2 - \omega^2 \vacuumpermeability \scalarpermittivity_{ord} \end{pmatrix} &\begin{pmatrix}\Efield_y\end{pmatrix} &=& 0
\end{align}

We call the solutions of the equation for $\Efield_y$ ordinary mode, and of the coupled system extraordinary mode.
We find the following eigenvalues and eigenvectors:

\subsubsection{ordinary mode}

\begin{eqnarray}
 \Wavevector^2 &=& \omega^2 \vacuumpermeability \scalarpermittivity_{ord} 
 \\
...
\begin{eqnarray}
 <\Vector{S}> = \frac{ |\Efield|^2 }{ 2\omega\vacuumpermeability } \Re \Wavevector
\end{eqnarray}


\subsubsection{extraordinary mode}
\begin{eqnarray}
  \frac{\wavenumber_{x}^2 }{\scalarpermittivity_{eo} } + \frac{\wavenumber_{z}^2 }{\scalarpermittivity_{ord} } &=& \omega^2 \vacuumpermeability 
  \label{eq:uniaxialAnisotropicDispersion}
  \\
  \scalarEfield_x \scalarpermittivity_{ord} \wavenumber_x &=& -  \scalarEfield_z \scalarpermittivity_{eo} \wavenumber_z 
\end{eqnarray}
...
\begin{eqnarray}
 ...
 &=& \frac{1}{2 \omega \vacuumpermeability} \Re 
  ...
  \\
 &=& \frac{1}{2 \omega \vacuumpermeability} \Re 
   \left( \frac{\wavenumber_x \scalarEfield_z \overline{\scalarEfield_z}}{\scalarpermittivity_{ord}} + \frac{\wavenumber_z \scalarEfield_x \overline{\scalarEfield_x}}{\scalarpermittivity_{eo}} \right)
   \begin{pmatrix}
     \scalarpermittivity_{ord} \wavenumber_x 
     \\
     0
     \\
...
\begin{eqnarray}
 \Wavevector^2 \scalarpermittivity_{ord} 
 + \wavenumber_{z}^2 (\scalarpermittivity_{eo} - \scalarpermittivity_{ord})
 &=& \omega^2 \vacuumpermeability \scalarpermittivity_{ord} \scalarpermittivity_{eo} 
 \\
 (\Wavevector_{1\,\parallel}^2 + \xi^2) \scalarpermittivity_{ord} 
 + (\Wavevector_{1\,\parallel} \Vector{e}_{eo} + \xi \Vector{n}\Vector{e}_{eo})^2 (\scalarpermittivity_{eo} - \scalarpermittivity_{ord})
 &=& \omega^2 \vacuumpermeability \scalarpermittivity_{ord} \scalarpermittivity_{eo}  
\end{eqnarray}
...
\begin{eqnarray}
 a \xi^2 + b \xi + c &=& 0
 \\
 a &=& \scalarpermittivity_{ord} + (\scalarpermittivity_{eo} - \scalarpermittivity_{ord}) (\Vector{n}\Vector{e}_{eo})^2
 \nonumber 
 \\
 b &=& 2 (\scalarpermittivity_{eo} - \scalarpermittivity_{ord}) (\Wavevector_{1\,\parallel} \Vector{e}_{eo}) (\Vector{n}\Vector{e}_{eo})
 \nonumber 
 \\
 c &=& \Wavevector_{1\,\parallel}^2 \scalarpermittivity_{ord} + (\scalarpermittivity_{eo} - \scalarpermittivity_{ord}) (\Wavevector_{1\,\parallel} \Vector{e}_{eo})^2 
       - \omega^2 \vacuumpermeability \scalarpermittivity_{ord} \scalarpermittivity_{eo} 
 \nonumber
\end{eqnarray}
...
\begin{eqnarray}
 \Vector{d} &=& \frac{<\Vector{S}>}{|<\Vector{S}>|}
\end{eqnarray}
